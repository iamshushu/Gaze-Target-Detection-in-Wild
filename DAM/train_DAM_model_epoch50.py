...
from model_train import GazeTargetDetectionNet
...
train_annotation_dir = '/home/soo/gaze_total/gaze/DAM/data/videoattentiontarget/annotations/train'
test_annotation_dir = '/home/soo/gaze_total/gaze/DAM/data/videoattentiontarget/annotations/test'
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for i, (image, depthmap, head, head_channel, gaze_field, heatmap, gaze_direction, inout, imsize, gaze_point, head_position) in enumerate(test_loader):
            input = image.cuda(), depthmap.cuda(), head.cuda(), head_channel.cuda(), gaze_field.cuda()
...
def train(args, logger):
    ...
    for i_epoch in tqdm.trange(args['number_epoch']):
        ...
        for i, (image, depthmap, head, head_channel, gaze_field, heatmap, gaze_direction, inout, imsize, gaze_point) in enumerate(train_loader):
            model.zero_grad()
            input = image.cuda(), depthmap.cuda(), head.cuda(), head_channel.cuda(), gaze_field.cuda()
        ...
        # 모델 저장하기
        torch.save(model.state_dict(), 'DAM_model_epoch50.pth')
        """
        **optional(not use)
        torch.save(model.state_dict(), 'DAM_model.pth')
        """
if __name__ == '__main__':
    ...
    logger = loggging(filename='DAM_model_epoch50.log')
    """
    **optional(not use)
    args = {
    'learning_rate':0.0001,
    ...
    logger = loggging(filename='DAM_model.log')
    """
