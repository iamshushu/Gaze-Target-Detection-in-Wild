...
class GazeTargetDetectionNet(nn.Module):
    ...
    def forward(self, x):
        img, depthmap, head, head_channel, gaze_field = x
        _, height, width = depthmap.size() 

        # 3D gaze direction estimation
        #for person in range(0, len(head[0])):
        gaze_input = head

        gaze_direction, _ = self.gazenet(gaze_input)
        
        z = torch.mul(torch.cos(gaze_direction[:, 1]), torch.cos(
            gaze_direction[:, 0]))
        z = z.view(-1, 1, 1)
        # print(z)
        x = torch.mul(torch.cos(gaze_direction[:, 1]), torch.sin(
            gaze_direction[:, 0]))
