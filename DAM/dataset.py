    ...
    def __getitem__(self, index):
        if self.test:
            ...
            for i in range(len(data)):
                path, head_x_min, head_y_min, head_x_max, head_y_max, gaze_x, gaze_y = data.iloc[i]
            ...
            path, head_x_min, head_y_min, head_x_max, head_y_max, gaze_x, gaze_y, inout = self.df.iloc[index]
        ...
        depth_path = path[:-4] + '-dpt_beit_large_512.png'
        #print(depth_path)
        ...
        if self.test:
            ...
            return image, depthmap, head, head_channel, gaze_field, gaze_collect, gaze_average, seperate_gaze_direction, average_gaze_direction, imsize, head_position
            ...
            return image, depthmap, head, head_channel, gaze_field, heatmap, gaze_direction, inout, imsize, gaze_point
...
class Videoatttarget_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, annotation_dir, depth_dir, test = False):
        ...
        #print('all_sequence_paths :', all_sequence_paths)
        #print(len(all_sequence_paths))
        df = []
        for seq in all_sequence_paths:
            #print(seq)
            sep_df = pd.read_csv(seq, sep=',', header=None, index_col=False, 
            names=['path', 'head_x_min', 'head_y_min', 'head_x_max', 'head_y_max', 'gaze_x', 'gaze_y']
            )
            #'gaze_1', 'gaze_2', 'gaze_3', 
    ...
    def __getitem__(self, index):
        path, head_x_min, head_y_min, head_x_max, head_y_max, gaze_x, gaze_y = self.df.iloc[index]
        ...
        if img is not None: 
            ...
            depth_path = path[:-4] +  '-dpt_beit_large_512.png'
            # print(depth_path)
            # gray scale 로 변경하는 코드 추가!!
            depthmap = Image.open(os.path.join(self.depth_dir, depth_path)).convert("L")
            ...
            if self.test:
                return image, depthmap, head, head_channel, gaze_field, heatmap, gaze_direction, inout, imsize, gaze_point, head_position
